...
BIB_KEYS: dict[str, str] = {
    "na23_relaxometry_2023": "na23_relaxometry_2023",
    "spin32_algebraic_2004": "spin32_algebraic_2004",
    "qst_relaxation_2008": "qst_relaxation_2008",
    "spin32_qlogic_qst_2005": "spin32_qlogic_qst_2005",
    "nonmarkov_noise_2022": "nonmarkov_noise_2022",
    "multipass_qpt_2024": "multipass_qpt_2024",
    "quadrupolar_qip_2012": "quadrupolar_qip_2012",
    "grape_nmr_control_2005": "grape_nmr_control_2005",
    "pps_optimal_control_2012": "pps_optimal_control_2012",
    "projected_ls_qpt_2022": "projected_ls_qpt_2022",
    "gate_set_tomography_2021": "gate_set_tomography_2021",
    "nonmarkov_process_tensor_2020": "nonmarkov_process_tensor_2020",
    "noise_filtering_control_2014": "noise_filtering_control_2014",
    "dd_noise_spectroscopy_2011": "dd_noise_spectroscopy_2011",
    "flux_qubit_noise_spectroscopy_2011": "flux_qubit_noise_spectroscopy_2011",
}
...
def primary_reference_latex(source_id: str) -> str:
    bib_key = BIB_KEYS.get(source_id)
    if bib_key:
        return rf"\textbf{{Primary reference.}} \citep{{{bib_key}}}."
    return rf"\textbf{{Primary reference.}} {latex_escape(reference_for_source(source_id))}"
...
def plot_figure_count(copied: dict[str, list[Path]]) -> Path:
    ...
    ax.set_title("Artifact Coverage across Reproductions and Workflows")
    ax.set_ylabel("number of plots included in the review article")
...
def plot_quality_metrics() -> Path:
    ...
    ax.set_title("Normalized Performance Indicators across the Benchmark Suite")
    ax.set_ylabel("normalized indicator (higher is better)")
...
def plot_improvement_factors() -> Path:
    ...
    ax.set_title("Improvement Factors Relative to Each Baseline Protocol")
...
def plot_error_metrics() -> Path:
    ...
    ax.set_yscale("log")
    ax.set_title("Representative Residual and Error Metrics across Benchmarks")
...
def write_comparison_sections(copied: dict[str, list[Path]], comparison_figures: list[Path]) -> None:
    lines = [
        ...
        r"\section{Results: Cross-Benchmark Comparative Analysis}",
        "",
        "This section compares the reproduced papers as a benchmark stack rather",
        "than as isolated articles.  The metrics are heterogeneous: fidelities,",
        "correlations, RMSE values, improvement factors, and physicality",
        "diagnostics do not represent the same observable.  The comparison is",
        "therefore interpretive rather than ordinal: it identifies which physical",
        "capability each reference contributes to the present laboratory.",
        "",
    ]
    for fig_path in comparison_figures:
        ...
        lines.extend(
                [
                    r"\begin{figure}[p]",
                    r"  \centering",
                    rf"  \includegraphics[width=0.92\textwidth,height=0.84\textheight,keepaspectratio]{{{latex_path(fig_path)}}}",
...
def write_expanded_sections(copied: dict[str, list[Path]]) -> None:
    lines = [
        "% Auto-generated by scripts/generate_review_article_assets.py",
        r"\section{Technical Appendix: Paper-by-Paper Reproduction and Reference Comparison}",
        "",
        "Each subsection below summarizes the scientific scope of the published",
        "reference, the implemented model, the quantitative outcome of the present",
        "reproduction, the current deviation from the published result, and the",
        "experimental implication for the laboratory.",
        "",
    ]
    for source_id in PAPER_ORDER:
        meta = PAPER_META[source_id]
        ...
        lines.extend(
            [
                ...
                "",
                primary_reference_latex(source_id),
                "",
                ...
                "",
                rf"\textbf{{Result and reference comparison.}} {latex_escape(commentary['laboratory_comparison'])}",
                "",
                rf"\textbf{{Interpretation for the present study.}} {latex_escape(meta['lesson'])}",
                "",
                rf"\textbf{{Experimental implication.}} {latex_escape(commentary['next_lab_use'])}",
                "",
                rf"\textbf{{Current deviation from the published reference.}} {latex_escape(meta['limitations'])}",
                "",
                rf"\textbf{{Reproduced figure count.}} {len(copied.get(source_id, []))}.",
                "",
            ]
        )
        if key_metrics:
...
def write_figure_atlas(copied: dict[str, list[Path]]) -> None:
    lines = [
        "% Auto-generated by scripts/generate_review_article_assets.py",
        r"\section{Technical Appendix: Complete Figure Atlas}",
        "",
...
def write_experimental_simulation_sections(base_records: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "% Auto-generated by scripts/generate_review_article_assets.py",
        r"\section{Technical Appendix: Baseline Validation and Experimental-Simulation Artifacts}",
        "",
        ...
        "auditable work.",
        "",
        r"\subsection{Quantitative Baseline Metrics}",
        "",
    ...
    lines.extend(
        [
            r"\subsection{Artifact Inventory}",
            "",
    ...
    for record in base_records:
        ...
        lines.append(
            rf"{latex_escape(record['label'])} & \path{{{record['source']}}} & {latex_escape(status)} & \texttt{{{latex_escape(sha)}}}\\"
        )
    lines.extend([r"\bottomrule", r"\end{longtable}", r"\normalsize", ""])

    lines.extend(
        [
            r"\subsection{Baseline and Simulation Figures}",
            "",
    ...
    for record in png_records:
        ...
        lines.extend(
            [
                r"\begin{figure}[p]",
                r"  \centering",
                rf"  \includegraphics[width=0.92\textwidth,height=0.84\textheight,keepaspectratio]{{{record['snapshot']}}}",
                rf"  \caption{{{latex_escape(latex_caption_from_note(note))}}}",
                r"\end{figure}",
                "",
            ]
