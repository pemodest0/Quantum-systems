...
ROOT = _project_root()
REPORT_DIR = ROOT / "reports" / "apostila_transporte_quantico_ptbr"
TEX_FILE = REPORT_DIR / "apostila_transporte_quantico_ptbr.tex"
...
def _generate_baseline_summary() -> None:
    ...
    lines = [
        "\\begin{table}[t]",
        "\\centering",
        "\\caption{Resumo automatico do baseline atual.}",
        "\\begin{tabular}{lcccc}",
        "\\toprule",
        "Cenario & Regime otimo & $\\eta_{\\mathrm{coh}}$ & $\\eta_{\\mathrm{best}}$ & $\\gamma_{\\phi}^{\\mathrm{best}}$ \\\\",
        "\\midrule",
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the PT-BR transport tutorial PDF.")
    ...
    pdf_path = REPORT_DIR / "apostila_transporte_quantico_ptbr.pdf"
    payload = {
